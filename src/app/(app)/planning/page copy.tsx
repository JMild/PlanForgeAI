...
const ProductionPlannerBoard: React.FC = () => {
  ...
  const [pxPerHour, setPxPerHour] = useState<number>(90); // (คงไว้ ใช้งานได้เลย)
  ...
  // Position helpers
  const dateToX = (dt: Date) => {
    const dayIdx = Math.floor((startOfDay(dt).getTime() - startOfDay(viewStart).getTime()) / MS_PER_DAY);
    const h = dt.getHours() + dt.getMinutes() / 60;
    return dayIdx * dayWidthPx + (h - DAY_START) * pxPerHour;
  };
  ...
  const handleAIPlan = (): void => {
      ...
      order.items.forEach((item) => {
        item.routing.forEach((step) => {
          const isScheduled = jobs.some(
            (j) => j.orderNo === order.orderNo && j.itemNo === item.itemNo && j.seq === step.seq
          );
          if (!isScheduled)
    ...
    const baseStart = new Date(anchorDate + "T08:00:00");
    MACHINES.forEach((m) => {
      machineNextAvailable[m.code] = new Date(baseStart);
    });

    allUnscheduledProcesses.forEach((process) => {
      if (process.seq > 1) {
        const prevJob = newJobs.find(
          (j) => j.orderNo === process.orderNo && j.itemNo === process.itemNo && j.seq === process.seq - 1
        );
        if (!prevJob) return;
      }
      ...
      if (startTime.getHours() < DAY_START) startTime.setHours(DAY_START, 0, 0, 0);
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-3">
            <button onClick={handleAIPlan} className="btn btn-primary">
              <Zap size={18} />
              AI Plan
            </button>
                                    ...
                                    return (
                                      <div key={step.seq} className="flex items-start gap-1">
                                        ...
                                        <div
                                          ...
                                          onDragStart={(e) =>
                                            canSchedule && !isScheduled && handleDragStart(e, order, item, step)
                                          }
                                          className={[
                              ...
                              <div key={`hours-${d}`}>
                                {Array.from({ length: (DAY_END - DAY_START) + 1 }, (_, i) => (
                                  <div
                                    ...
                                    style={{ left: d * dayWidthPx + i * pxPerHour }}
                                  />
                                ))}
                              </div>
                            ))}
                            ...
                            .map((job) => {
                              ...
                              return (
                                <div
                                  ...
                                  title={hasConflict ? `Conflicts: ${types.join(", ")}` : "Click to remove"}
                                >
