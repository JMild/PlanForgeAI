...
import {
  Plus, Search, Edit, Trash2, Eye, Download, Upload, Copy,
  Package, GitBranch, Clock, Tag, AlertCircle, CheckCircle,
  X, Save, ChevronRight, FileText, Layers, ArrowRight
} from 'lucide-react';
...
const CATEGORIES = ['Widgets', 'Components', 'Assemblies', 'Raw Materials', 'Finished Goods'];
const UNITS = ['pcs', 'kg', 'L', 'M', 'm²', 'set'];
...
const ProductMasterData = () => {
  ...
  const [activeTab, setActiveTab] = useState('basic'); // 'basic', 'routing', 'bom'
  ...
  const openModal = (mode, product = null) => {
    ...
    if (mode === 'create') {
      setFormData({
        code: `WDGT-${String.fromCharCode(65 + products.length)}`,
        name: '',
        description: '',
        category: 'Widgets',
        unit: 'pcs',
  ...
  const handleSave = () => {
    if (modalMode === 'create') {
      ...
      setProducts(products.map(p => p.code === selectedProduct.code ? formData : p));
    }
    closeModal();
  };
  ...
  const handleDelete = (productCode) => {
    if (confirm(`Are you sure you want to delete product ${productCode}?`)) {
      setProducts(products.filter(p => p.code !== productCode));
    }
  };
  ...
  const addRoutingStep = () => {
    setFormData({
      ...
      routing: [
        ...formData.routing,
        { seq: formData.routing.length + 1, process: '', processName: '', setupMin: 0, runMinPerUnit: 0, machineGroup: '' }
      ]
    });
  };
  ...
  const removeRoutingStep = (index) => {
    const newRouting = formData.routing.filter((_, i) => i !== index).map((step, i) => ({
      ...step,
      seq: i + 1
    }));
    setFormData({ ...formData, routing: newRouting });
  ...
  const calculateTotalProcessTime = (routing, qty = 1) => {
    return routing.reduce((total, step) => total + step.setupMin + (step.runMinPerUnit * qty), 0);
  };
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Manage products, routing, and bill of materials</p>
            </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Lot Size</th>
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Lead Time</th>
                ...
                {filteredProducts.map(product => (
                  <tr key={product.code} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-right">
                      <span className="text-sm text-gray-900">{product.lotSize} {product.unit}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex text-xs px-2 py-1 rounded ${product.status === 'Active' ? 'bg-green-100 text-green-700' :
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="px-6 border-b border-gray-200 flex gap-4">
              {['basic', 'routing', 'bom'].map(tab => (
                ...
                >
                  {tab === 'basic' && <span className="flex items-center gap-2"><Package size={16} /> Basic Info</span>}
                  {tab === 'routing' && <span className="flex items-center gap-2"><GitBranch size={16} /> Routing ({formData.routing.length} steps)</span>}
                  {tab === 'bom' && <span className="flex items-center gap-2"><Layers size={16} /> BOM ({formData.bom.length} items)</span>}
                </button>
              ))}
            </div>
                            ...
                            <div className="flex-1 grid grid-cols-2 gap-4">
                              ...
                              <div>
                                ...
                                <input
                                  type="number"
                                  value={step.setupMin}
                                  onChange={(e) => updateRoutingStep(index, 'setupMin', parseFloat(e.target.value) || 0)}
                                  disabled={modalMode === 'view'}
                                  className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
                                  min="0"
                                />
                              ...
                              <div>
                                <label className="block text-xs font-medium text-gray-700 mb-1">Run Time per Unit (min)</label>
                                <input
                                  type="number"
                                  value={step.runMinPerUnit}
                                  onChange={(e) => updateRoutingStep(index, 'runMinPerUnit', parseFloat(e.target.value) || 0)}
                                  disabled={modalMode === 'view'}
                                  className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
                                  min="0"
                                  step="0.1"
                  ...
                  {formData.routing.length > 0 && (
                    <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                      <div className="text-sm text-blue-900">
                        ...
                        {formData.lotSize > 1 && (
                          <span className="ml-2">
                            ({calculateTotalProcessTime(formData.routing, formData.lotSize)} min for lot of {formData.lotSize})
                          </span>
                        )}
                      </div>
                    </div>
                          ...
                          {formData.bom.map((item, index) => (
                            <tr key={index} className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3">
                                <select
                                  value={item.unit}
                                  onChange={(e) => updateBOMLine(index, 'unit', e.target.value)}
                                  disabled={modalMode === 'view'}
                              ...
                              <td className="px-4 py-3">
                                <input
                                  ...
                                  onChange={(e) => updateBOMLine(index, 'scrapRate', parseFloat(e.target.value) || 0)}
                                  disabled={modalMode === 'view'}
