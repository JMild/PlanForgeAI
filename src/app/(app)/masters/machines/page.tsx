...
import {
  ...
  Save, X, ChevronDown, ChevronRight, Cpu, AlertCircle, Activity,
  Calendar, Wrench, Clock, Zap, TrendingUp, CheckCircle
} from 'lucide-react';
...
// Sample processes
const PROCESSES = [
  'MACH', 'DRILL', 'PRESS', 'PAINT', 'ASSY', 'PACK', 'WELD', 'INSP', 'MILL', 'TURN'
];
...
const MachinesMasterData = () => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'Running': return <Activity className="text-green-600" size={16} />;
      case 'Idle': return <Clock className="text-gray-600" size={16} />;
      case 'PM': return <Wrench className="text-yellow-600" size={16} />;
      case 'Down': return <AlertCircle className="text-red-600" size={16} />;
      case 'Setup': return <Settings className="text-blue-600" size={16} />;
      default: return <Cpu className="text-gray-600" size={16} />;
    }
  };

  const getOEEColor = (oee, target) => {
    if (oee >= target) return 'text-green-600';
    if (oee >= target * 0.9) return 'text-yellow-600';
    return 'text-red-600';
  };

  const getMaintenanceStatus = (dueDays) => {
  ...
  const filteredMachines = machines.filter(machine => {
    const matchesSearch = machine.code.toLowerCase().includes(searchTerm.toLowerCase()) ||
      machine.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      machine.description.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesWorkCenter = filterWorkCenter === 'all' || machine.workCenterCode === filterWorkCenter;
    const matchesStatus = filterStatus === 'all' || machine.status === filterStatus;
    return matchesSearch && matchesWorkCenter && matchesStatus;
  });

  const toggleMachineExpand = (code) => {
  ...
  const handleSaveMachine = () => {
    ...
    if (editingMachine) {
      setMachines(machines.map(m => m.code === editingMachine.code ? newMachine : m));
    } else {
  ...
  const handleDeleteMachine = (code) => {
    if (confirm(`Are you sure you want to delete machine ${code}?`)) {
      setMachines(machines.filter(m => m.code !== code));
    }
  };
                    ...
                    <div className="p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4 flex-1">
                          <button
                            onClick={() => toggleMachineExpand(machine.code)}
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              <span className={`text-xs px-2 py-1 rounded flex items-center gap-1 ${getStatusColor(machine.status)}`}>
                            ...
                            <div className="space-y-2 text-sm">
                              <div>
                                ...
                                <div className="text-gray-900">
                                  {new Date(machine.lastPMDate).toLocaleDateString()}
                                </div>
                              </div>
                              <div>
                                <span className="text-gray-500">Next PM:</span>
                                <div className="text-gray-900">
                                  {new Date(machine.nextPMDate).toLocaleDateString()}
                                </div>
                              </div>
                              <div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-3 gap-6">
                    ...
                    <div>
                      ...
                      <p className="mt-1">
                        <span className={`text-sm px-3 py-1 rounded flex items-center gap-1 w-fit ${getStatusColor(editingMachine.status)}`}>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-5 gap-2">
                      {PROCESSES.map(proc => (
                        <label
                          key={proc}
                          className={`flex items-center justify-center px-3 py-2 border-2 rounded-lg cursor-pointer transition-colors ${formData.processes.includes(proc)
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setFormData({ ...formData, capacityHoursPerDay: parseFloat(e.target.value) || 0 })}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                          min="0"
                          max="24"
                          step="0.5"
