...
import {
  ...
  Calendar, Filter, FileText, PieChart as PieChartIcon, Activity,
  ChevronDown, ChevronRight, Printer, Mail, RefreshCw
} from 'lucide-react';
import {
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Analytics and insights for production performance</p>
            </div>
            ...
            <div className="px-6 py-4 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {selectedReportInfo && <selectedReportInfo.icon size={24} className="text-blue-600" />}
                  <div>
                    <h2 className="text-xl font-semibold text-gray-900">{selectedReportInfo?.name}</h2>
                    <p className="text-sm text-gray-500">Period: {dateRange === 'week' ? 'This Week' : dateRange === 'month' ? 'This Month' : 'Custom'}</p>
                  </div>
                </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
          <div className="text-sm text-blue-600 font-medium mb-1">Average Utilization</div>
        ...
        <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
          <div className="text-sm text-purple-600 font-medium mb-1">Active Machines</div>
          <div className="text-3xl font-bold text-purple-900">{data.filter(m => m.utilization > 0).length}</div>
        </div>
      </div>

      {/* Chart */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Utilization & OEE by Machine</h3>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Avg Wait Time</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {data.map(process => (
                <tr key={process.process} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-right text-gray-900">{process.throughput} units</td>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
          <div className="text-sm text-orange-600 font-medium mb-1">Avg Changeover Time</div>
      ...
      {/* Chart */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Changeover Analysis by Machine</h3>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data}>
            <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
            <XAxis dataKey="machine" stroke="#6B7280" style={{ fontSize: '12px' }} />
      ...
      {/* Detailed Table */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Machine Changeover Details</h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
            ...
            <tbody className="divide-y divide-gray-200">
              {data.map(machine => (
                <tr key={machine.machine} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-sm font-medium text-gray-900">{machine.machine}</td>
                  <td className="px-4 py-3 text-sm text-right text-gray-900">{machine.avgChangeover} min</td>
                  <td className="px-4 py-3 text-sm text-right text-gray-900">{machine.changeoverCount}</td>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
          <div className="text-sm text-orange-600 font-medium mb-1">Avg Days Late</div>
          <div className="text-3xl font-bold text-orange-900">{avgDaysLate}</div>
        </div>
        <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
          <div className="text-sm text-yellow-600 font-medium mb-1">Priority 1 Late</div>
          <div className="text-3xl font-bold text-yellow-900">{data.filter(o => o.priority === 1).length}</div>
        </div>
      </div>

      {/* Late Orders Table */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Late Order Details</h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Days Late</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {data.map(order => (
                <tr key={order.orderNo} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-right">
                    <span className="font-medium text-red-600">{order.daysLate} day{order.daysLate !== 1 ? 's' : ''}</span>
                  </td>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-green-50 p-4 rounded-lg border border-green-200">
          ...
          <div className="text-3xl font-bold text-green-900">{onTargetPercentage}%</div>
        </div>
        <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
          <div className="text-sm text-purple-600 font-medium mb-1">Total Orders</div>
          <div className="text-3xl font-bold text-purple-900">{data.reduce((sum, p) => sum + p.orders, 0)}</div>
        </div>
      </div>

      {/* Chart */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Lead Time vs Target by Product</h3>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data}>
            <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
      ...
      {/* Detailed Table */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Product Lead Time Analysis</h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Product</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Avg Lead Time</th>
              ...
              {data.map(product => {
                ...
                return (
                  <tr key={product.product} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-right text-gray-900">{product.avgLeadTime} min</td>
                    <td className="px-4 py-3 text-sm text-right text-gray-600">{product.targetLeadTime} min</td>
                    <td className="px-4 py-3 text-sm text-right">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
          <div className="text-sm text-yellow-600 font-medium mb-1">Avg per Incident</div>
          <div className="text-3xl font-bold text-yellow-900">{Math.round(totalDowntime / totalIncidents)} min</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div>
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Duration by Reason</h3>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={data} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
              <XAxis type="number" stroke="#6B7280" style={{ fontSize: '12px' }} />
              <YAxis dataKey="reason" type="category" stroke="#6B7280" style={{ fontSize: '11px' }} width={120} />
              <Tooltip />
            ...
            <tbody className="divide-y divide-gray-200">
              {data.map((reason, index) => (
                <tr key={reason.reason} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-right text-gray-900">{reason.duration} min</td>
                  <td className="px-4 py-3 text-sm text-right text-gray-900">{reason.count}</td>
...
const OnTimeDeliveryReport = ({ data }) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-red-50 p-4 rounded-lg border border-red-200">
          ...
          <div className="text-3xl font-bold text-red-900">{totalOrders - totalOnTime}</div>
        </div>
      </div>

      {/* Chart */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">On-Time Delivery Trend</h3>
        <ResponsiveContainer width="100%" height={300}>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Late</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {data.map(week => (
                <tr key={week.week} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-right text-gray-900">{week.total}</td>
                  <td className="px-4 py-3 text-sm text-right text-green-600 font-medium">{week.onTime}</td>
...
const PlanAdherenceReport = ({ data }) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
          ...
          <div className="text-3xl font-bold text-blue-900">{adherencePercentage}%</div>
          <div className="text-xs text-blue-600 mt-1">Within ±2 jobs tolerance</div>
        </div>
        <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
        ...
        <div className="bg-green-50 p-4 rounded-lg border border-green-200">
          ...
          <div className="text-3xl font-bold text-green-900">{data.reduce((sum, d) => sum + d.planned, 0)}</div>
        </div>
      </div>

      {/* Chart */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Planned vs Actual Jobs</h3>
        <ResponsiveContainer width="100%" height={300}>
          <ComposedChart data={data}>
            <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
...
const BottleneckReport = ({ data }) => {
  const topBottleneck = data.reduce((max, p) => p.avgWaitTime > max.avgWaitTime ? p : max);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-red-50 p-4 rounded-lg border border-red-200">
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="font-semibold text-red-900 mb-1">Primary Bottleneck Identified</div>
            <div className="text-sm text-red-800">
              <strong>{topBottleneck.process}</strong> has the highest average wait time of <strong>{topBottleneck.avgWaitTime} minutes</strong> with {topBottleneck.utilization}% utilization
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Chart */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Average Wait Time by Process</h3>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data} layout="vertical">
            <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
            <XAxis type="number" stroke="#6B7280" style={{ fontSize: '12px' }} />
            <YAxis dataKey="process" type="category" stroke="#6B7280" style={{ fontSize: '12px' }} width={100} />
            <Tooltip />
      ...
      {/* Detailed Table */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Process Analysis</h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Planned (min)</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Actual (min)</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {data.map(machine => (
                <tr key={machine.machine} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-sm font-medium text-gray-900">{machine.machine}</td>
                  <td className="px-4 py-3 text-sm text-right text-gray-900">{machine.utilization}%</td>
                  <td className="px-4 py-3 text-sm text-right text-gray-900">{machine.oee}%</td>
                  <td className="px-4 py-3 text-sm text-right text-gray-600">{machine.planned}</td>
                    ...
                    <span className={`text-xs px-2 py-1 rounded ${machine.utilization >= 80 ? 'bg-green-100 text-green-700' :
                      ...
                      {machine.utilization >= 80 ? 'Excellent' :
                        machine.utilization >= 60 ? 'Good' :
                          machine.utilization > 0 ? 'Low' : 'Down'}
                    </span>
                  </td>
                </tr>
              ))}
