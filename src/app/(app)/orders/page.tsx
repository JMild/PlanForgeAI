...
import {
  ...
  Package, Calendar, User, Paperclip, X, Save, ChevronDown,
...
const INITIAL_ORDERS = [
  ...
  },
  {
    ...
    items: [
      { itemNo: 1, productCode: 'WDGT-C', productName: 'Widget C', qty: 75, notes: '' },
    ],
    notes: '',
    attachments: [],
  },
...
const OrderManagement = () => {
  ...
  const filteredOrders = orders.filter(order => {
    ...
    const matchesPriority = filterPriority === 'all' || order.priority === parseInt(filterPriority);
  ...
  const handleSaveOrder = () => {
    ...
    if (editingOrder) {
      setOrders(orders.map(o => o.orderNo === editingOrder.orderNo ? newOrder : o));
    } else {
  ...
  const addItem = () => {
    setFormData({
      ...formData,
      items: [...formData.items, { itemNo: formData.items.length + 1, productCode: '', qty: 0, notes: '' }]
    });
  };
  ...
  const removeItem = (itemNo) => {
    ...
    setFormData({
      ...formData,
      items: formData.items.filter(i => i.itemNo !== itemNo).map((item, idx) => ({
        ...item,
        itemNo: idx + 1
      }))
    });
  ...
  const calculateTotalProcessTime = (productCode, qty) => {
    const product = PRODUCT_CATALOG.find(p => p.code === productCode);
    if (!product) return 0;

    return product.routing.reduce((total, step) => {
      return total + step.setupMin + (step.runMinPerUnit * qty);
    }, 0);
  };
          ...
          <div className="px-6 py-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Create and manage production orders</p>
              </div>
          ...
          {filteredOrders.length === 0 ? (
            ...
            <div className="divide-y divide-gray-200">
              {filteredOrders.map(order => {
                ...
                return (
                  <div key={order.orderNo} className="hover:bg-gray-50 transition-colors">
                    <div className="p-4">
                      <div className="flex items-center justify-between">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-6 text-sm text-gray-600">
                              ...
                              {order.attachments.length > 0 && (
                                <div className="flex items-center gap-1">
                                  <Paperclip size={14} />
                                  <span>{order.attachments.length} file{order.attachments.length !== 1 ? 's' : ''}</span>
                                </div>
                              )}
                            </div>
                          </div>
                      ...
                      {isExpanded && (
                        <div className="mt-4 ml-12 space-y-2">
                          ...
                          {order.items.map(item => {
                            const product = PRODUCT_CATALOG.find(p => p.code === item.productCode);
                            const totalTime = calculateTotalProcessTime(item.productCode, item.qty);

                            return (
                              <div key={item.itemNo} className="border border-gray-200 rounded-lg p-3 bg-white">
                                <div className="flex items-start justify-between">
                                  <div className="flex-1">
                                    ...
                                    <div className="text-sm text-gray-600 space-y-1">
                                      <div>Quantity: <span className="font-medium">{item.qty} units</span></div>
                                      <div>Est. Process Time: <span className="font-medium">{Math.round(totalTime)} minutes</span></div>
                                      {item.notes && (
                                    ...
                                    {product && (
                                      <div className="mt-2 pt-2 border-t border-gray-100">
                                        <div className="text-xs text-gray-500 mb-1">Routing Steps:</div>
                                        <div className="flex items-center gap-2">
                          ...
                          {order.notes && (
                            <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                              <div className="flex items-start gap-2">
                                ...
                                <div>
                                  <div className="text-xs font-medium text-yellow-900 mb-1">Order Notes:</div>
              ...
              {viewMode === 'view' && editingOrder ? (
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-gray-900">{new Date(editingOrder.createdDate).toLocaleDateString()}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-gray-700">Due Date</label>
                      <p className="mt-1 text-gray-900">{new Date(editingOrder.dueDate).toLocaleDateString()}</p>
                    </div>
                  </div>

                  <div>
                    <label className="text-sm font-medium text-gray-700 mb-3 block">Order Items</label>
                      ...
                      {editingOrder.items.map(item => {
                        ...
                        return (
                          <div key={item.itemNo} className="border border-gray-200 rounded-lg p-4">
                            ...
                            <p className="text-sm text-gray-700">Quantity: {item.qty} units</p>
                            {item.notes && <p className="text-sm text-blue-600 mt-1">Note: {item.notes}</p>}
                            {product && (
                              <div className="mt-2 pt-2 border-t">
                                <div className="text-xs text-gray-500 mb-1">Routing:</div>
                                <div className="flex gap-2 flex-wrap">
                                  {product.routing.map(step => (
                                    <span key={step.seq} className="text-xs px-2 py-1 bg-blue-50 text-blue-700 rounded">
                                      {step.processName}
                                    </span>
                                  ))}
                                </div>
                  ...
                  {editingOrder.notes && (
                    <div>
                      ...
                      <p className="mt-1 text-gray-900 p-3 bg-gray-50 rounded">{editingOrder.notes}</p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div>
                      <label className="text-sm font-medium text-gray-700 block mb-2">Customer *</label>
                      <select
                      ...
                      >
                        ...
                        {CUSTOMERS.map(customer => (
                          <option key={customer.code} value={customer.code}>{customer.name}</option>
                        ))}
                      </select>
                    </div>
                    <div>
                  ...
                  <div>
                    <label className="text-sm font-medium text-gray-700 block mb-2">Order Notes</label>
                    <textarea
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {formData.items.map(item => {
                        const product = PRODUCT_CATALOG.find(p => p.code === item.productCode);
                        return (
                          <div key={item.itemNo} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                              ...
                              <div className="flex-1 grid grid-cols-3 gap-3">
                                ...
                                <div>
                                  ...
                                  <input
                                    type="number"
                                    value={item.qty}
                                    onChange={(e) => updateItem(item.itemNo, 'qty', parseInt(e.target.value) || 0)}
                                    className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    min="1"
                                    required
                                  />
                            ...
                            {product && (
                              <div className="mt-2 pt-2 border-t border-gray-200">
                                ...
                                <div className="text-xs text-gray-600 mt-1">
                                  Est. Time: {Math.round(calculateTotalProcessTime(item.productCode, item.qty))} min
                                </div>
                              </div>
                            )}
                          </div>
